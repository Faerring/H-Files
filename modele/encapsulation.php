...
class execRequest
{
    ...
    {
        $y = "SELECT IDActe, nomActe FROM acte NATURAL JOIN affectation NATURAL JOIN hospitalisation WHERE UUID LIKE '" . $patientID . "' AND IDHosp = " . $hosp;
        $actes = $dbh->query($y);
        return $actes;
    }
    ...
    public static function addHospitalisation($enterDate, $outDate, $UUID, $service, $dbh)
    {
        $outDate = empty($outDate) ? NULL : $outDate;
        $x = 'INSERT INTO `hospitalisation` (`IDHosp`, `DateHostpitalisation`, `DateFin`, `UUID`) VALUES (NULL,"' . $enterDate . '","' . $outDate . '","' . $UUID . '");';
        return $dbh->query($x);
    }
